...
>
    <div class="relative transition-transform duration-300 ease-out" style="transform: translate(-50%, -50%) scale({isHovering ? 1.5 : 1});">
        <div class="absolute inset-[-3px] rounded-full border border-[#E1E0CC]/30 transition-all duration-300 {isHovering ? 'border-[#E1E0CC]/60 scale-125' : ''}"></div>
        <div class="w-[40px] h-[40px] rounded-full overflow-hidden border-2 border-white/30 shadow-[0_0_15px_rgba(212,163,115,0.5)] transition-all duration-300">
            <img src="/loge.png" alt="" class="w-full h-full object-cover">
        </div>
    </div>
</div>

<div class="fixed inset-0 pointer-events-none -z-10 bg-[#0E0C09] overflow-hidden">
    <!-- Animated Glowing Blobs - Dark Theme -->
    <div class="absolute top-0 left-0 w-full h-full overflow-hidden opacity-40">
        <!-- Blue blob top-left -->
        <div class="absolute top-[-15%] left-[-10%] w-[45%] h-[45%] rounded-full blur-[120px] animate-blob" style="background: #4A3220;"></div>
        <!-- Cyan blob bottom-left -->
        <div class="absolute bottom-[-15%] left-[-5%] w-[40%] h-[40%] rounded-full blur-[100px] animate-blob" style="background: linear-gradient(135deg, #8A5A2B, #5C3F27); animation-delay: 2s;"></div>
        <!-- Blue blob top-right -->
        <div class="absolute top-[10%] right-[-10%] w-[40%] h-[40%] rounded-full blur-[110px] animate-blob" style="background: #A0622D; animation-delay: 4s;"></div>
        <!-- Indigo blob bottom-right -->
        <div class="absolute bottom-[5%] right-[5%] w-[25%] h-[25%] rounded-full blur-[80px] animate-blob" style="background: #2E2317; animation-delay: 6s;"></div>
    </div>
    <!-- Grid Overlay -->
